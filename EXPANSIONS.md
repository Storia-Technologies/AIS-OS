...
| Folder / file | Purpose |
|---|---|
| `context/` | About you, your business, your priorities. Filled by [[.claude/skills/onboard/SKILL\|/onboard]]. |
| `references/` | Frameworks, voice samples, API guides, SOPs as you build them. |
| [[log]] | Append-only record of what was decided and why. |
| `archives/` | Old files. Don't delete — move here. |
| [[connections]] | Registry of every system your AIOS can reach. |
| `.claude/skills/` | Your skills: [[.claude/skills/onboard/SKILL\|/onboard]], [[.claude/skills/audit/SKILL\|/audit]], [[.claude/skills/level-up/SKILL\|/level-up]]. Add more via [[.claude/skills/level-up/SKILL\|/level-up]]. |
| [[aios-intake]] | Source-of-truth for [[.claude/skills/onboard/SKILL\|/onboard]]. Edit and re-run any time. |
| [[CLAUDE]] | Root operating manual. Filled by [[.claude/skills/onboard/SKILL\|/onboard]]. Edit when your role/voice changes. |
...
| `references/{tool}-api.md` | You connect a new API or MCP and figure out how it works | Researched-once-saved-forever. [[.claude/skills/audit/SKILL\|/audit]] rewards this; future skills don't re-research. |
...
- [[log]] — every meaningful decision ([[.claude/skills/level-up/SKILL|/level-up]] Phase 2 captures these automatically)
...
- [[connections]] — every time a new tool gets wired in, add a row
- `references/{tool}-api.md` — same time as [[connections]] update; capture the API once
- [[CLAUDE]] — quarterly review; rewrite the persona/priorities section after [[.claude/skills/level-up/SKILL|/level-up]] Q90
...
- **Don't have parallel `decisions.md` and [[log]].** Pick one. The kit ships [[log]].
...
3. **Could [[.claude/skills/level-up/SKILL|/level-up]] route a future skill into here naturally?** If yes, the AIOS will use it. If no, you're organizing for yourself, not for the system.
